public class MooreRules extends Rules { // The MooreRules class specifies its rules using boolean arrays of length 9, where the index of each element that's set to true indicates the quantity of neighbors that satisfies that rule. For example, if the elements at index 2 and 3 in the survivalRules array are true, it means that a live cell survives if it has 2 or 3 live neighbors surrounding it.
    ...
    public MooreRules(int[] birthNeighbors, int[] survivalNeighbors) {
        ...
        for (int neighbors: birthNeighbors) {
            birthRules[neighbors] = true; //birthRules[neighbors] is an index
        }
